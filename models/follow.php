 ...
 class Follow extends User{
	...
	public function followBtn($profileID, $user_id, $followID){
		...
		if($this->loggedIn()===true){
			if($profileID != $user_id){
				...
				//edit button
				return "<button class='new-btn' onclick=location.href='".BASE_URL.'views/'."profileEdit.php' style='outline:none;'>Edit Profile</button>";
			}
	...
	public function followingList($profileID, $user_id, $followID){
		...
		foreach ($followings as $following) {
			echo '<div class="following-box followingD">
					...
					<div class="follow-unfollow-inner">
						...
						<div class="follow-person-button-img mt-2">
							<div class="follow-person-img mr-4"> 
							 	<img src="'.BASE_URL.'views/'.$following->profileImage.'"/>
							</div>
                            <div class="follow-person-name mt-2">
								<a href="profile.php?username='.$following->username.'">'.$following->screenName.'</a>
							</div>
							<div class="follow-person-tname">
								<a href="profile.php?username='.$following->username.'">@'.$following->username.'</a>
							</div>
	...
	public function whoToFollow($user_id, $profileID){
		...
		echo '<div class="trends_container"><div class="trends_box rightdark" id="right-down"><div class="trends_header"><p>Who to follow</p></div>';
			...
			echo '<div class="follow-body trend">
					...
					<div class="media-inner">
						<div class="fo-co-head media-body">
							<a href="'.BASE_URL.'views/profile.php?username='.$user->username.'">'.$user->screenName.'</a><br><span>@'.$user->username.'</span>
						</div>
						<!-- FOLLOW BUTTON -->
