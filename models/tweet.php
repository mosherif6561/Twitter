...
class Tweet extends User{
	...
	public function tweetsHome($user_id, $num){
	      ...
	      $user = $this->userData($tweet->retweetBy);
 	      echo '<div class="all-tweet">
			      <div class="t-show-wrap">
			       ...
			       '.((isset($retweet['retweetID']) ? $retweet['retweetID'] === $tweet->retweetID OR $tweet->retweetID > 0 : isset($user->screenName)) ? '
			          ...
			          <div class="t-show-img">
			        	...
			        	<div class="t-s-head-content">
			        		<div class="t-h-c-name">
			        			<span><a href="profile.php?username='.$user->username.'">'.$user->screenName.'</a></span>
			        ...
			        <div class="t-s-b-inner">
			            ...
			            '.((!empty($tweet->tweetImage)) ? '
			        			<div class="retweet-t-s-b-inner-left">
			        				<img src="'.BASE_URL.'views/'.$tweet->tweetImage.'" class="imagePopup" data-tweet="'.$tweet->tweetID.'"/>
			        			</div>' : '').'
			        			<div>
			        				<div class="t-h-c-name">
			        					<span><a href="profile.php?username='.$tweet->username.'">'.$tweet->screenName.'</a></span>
			      	...
			      	<div class="t-show-footer">
			      		<div class="t-s-f-right">
			      			<ul>	
			      				<li>
								  <button 
								  class="t-show-popup" id="postComment"
									data-user="'.$tweet->tweetBy.'" data-tweet = "'.$tweet->tweetID.'" style="outline:none;"><i class="fa fa-comment" aria-hidden="true"></i>
	...
	public function tweetsVisitor(){
	    $stmt = $this->pdo->prepare("SELECT * FROM `tweets` LEFT JOIN `users` ON `tweetBy` = `user_id` AND `retweetID` = '0' ORDER BY `tweetID` DESC");
	    $stmt->execute();
	    $tweets = $stmt->fetchAll(PDO::FETCH_OBJ);
	    foreach ($tweets as $tweet) {
		  echo '
	...
	public function tweets($username, $num){
		$stmt = $this->pdo->prepare('SELECT `user_id` FROM `users` WHERE `username` = :username');
		$stmt->bindParam(':username', $username, PDO::PARAM_STR);
		$stmt->execute();
		$user = $stmt->fetch(PDO::FETCH_OBJ);
		$user_id = $user->user_id;
	    $stmt = $this->pdo->prepare("SELECT * FROM `tweets` LEFT JOIN `users` ON `tweetBy` = `user_id` WHERE `tweetBy` = :user_id AND `retweetID` >= '0' or `retweetBy` = :user_id ORDER BY `tweetID` DESC LIMIT :num");
		$stmt->bindParam(":user_id", $user_id, PDO::PARAM_INT);
	    $stmt->bindParam(":num", $num, PDO::PARAM_INT);
	    $stmt->execute();
	    $tweets = $stmt->fetchAll(PDO::FETCH_OBJ);
	...
	public function editTweet($tweet_id, $status){
		$stmt = $this->pdo->prepare("UPDATE `tweets` SET `status` = :status WHERE `tweetID` = :tweet_id");
	...
	public function addComment($user_id, $tweet_id, $get_id, $comment){
		$this->create('comments', array('comment' => $comment, 'commentBy' => $user_id, 'commentOn' => $tweet_id));
	
		if($get_id != $user_id){
			$this->message->sendNotification($get_id, $user_id, $tweet_id, 'comment');
		}
	}
	...
	public function getTweetLinks($tweet){
		...
		$tweet = preg_replace("/@([\w]+)/", "<a href='http://localhost/twitter/views/profile.php?username=$1'>$0</a>", $tweet);
		return $tweet;		
	}
	...
	public function trends(){
		$stmt = $this->pdo->prepare("SELECT *, COUNT(`tweetID`) AS `tweetsCount` FROM `trends` INNER JOIN `tweets` ON `status` LIKE CONCAT('%#',`hashtag`,'%') OR `retweetMsg` LIKE CONCAT('%#',`hashtag`,'%') GROUP BY `hashtag` ORDER BY `tweetID` LIMIT 4");
		$stmt->execute();	
		$trends = $stmt->fetchAll(PDO::FETCH_OBJ);
		echo '<div class="trends_container"><div class="trends_box rightdark" id="right-top"><div class="trends_header"><p>Trends for you</p></div><!-- trend title end-->';
		...
		echo '<div class="trends_show-more">
                    <a href="../views/hashtag.php">Show more</a>
                </div></div></div>';		
	} 
}
?>